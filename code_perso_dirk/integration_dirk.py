...
def DIRK_integration(f, y0, t_span, nt, A, b, c, options, gradF=None):
    ...
    s = np.size(b)
    bRosenbrockApprox = False # to perform approximate bu faster resolution of the substeps
    bUseCustomNewton = True # use custm newton solver instead of scipy method
                    ...
                    if bUseCustomNewton:
                        kni, Dres, LU, Dresinv = solver.solveNewton(fun=tempfun,
                                                    ...
                                                    options={'eps':1e-8, 'bJustOutputJacobian':False, 'nIterMax':50, 'bVectorisedModelFun':False,
                                                             'bUseComplexStep':True, 'bUseLUdecomposition':True, 'bUseInvertJacobian':False,
